function y_pred = predict_m_steps(A, B, y, u_data, m, N, k)
    ...
    for t = numel(B)+max([numel(S),numel(B),numel(R),offset + numel(B)])+k:N + 1 * numel(B) + offset
        y_pred(t+m-offset) =  R * u_sampl(t+m+(1-numel(B))-k-offset-(1:numel(R))) + S * y_sampl(t-offset-(0:numel(S)-1));
    end
    % for t = numel(B)+max([numel(S),numel(B),numel(R),offset + numel(B)])+k:N + 1 * numel(B) + offset
    %     y_pred(t+m-offset) =  R * u_sampl(t+m-k-offset-(0:numel(R)-1)) + S * y_sampl(t-offset-(0:numel(S)-1));
    % end
